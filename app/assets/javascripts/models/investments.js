App.Models.Investments = Backbone.Model.extend({
  ...
  syncParams: function(fields){
    ...
    console.log('TODO: investments background');
    return false;
  ...
  isFilled: function(){
    ...
    return App.bigDecision.get('monthly_savings') == total;
  },
  ...
  calcOpportunities: function(force){
    ...
    ['credit_cards', 'student_loans', 'other_debts'].forEach((kind)=>{
      ...
      let taxExempt = kind != 'student_loans';
    ...
    if (mortgage > 0){
      opps.push({title: 'Mortgage', balance: mortgage, grossReturn: App.finAssumptions.get('mortgage_rate'),
          isDebt: true, field: 'mortgage'})
    }
    ...
    var efundTarget = this.getShortEfundTarget();
    console.log(efundTarget);
    opps.push({field: 'match401k',             title: '401k (Match)',
               balance: sumMatch401,           grossReturn: rt_avg - k167, isTaxDeffered: true});
    opps.push({field: 'unmatched401k',         title: '401k (Unmatched)',
               balance: k163+l163-sumMatch401, grossReturn: rt_avg - k167, isTaxDeffered: true});
    ...
    opps.push({field: 'ira_traditional',       title: 'IRA (Traditional)',
               balance: 5500,                  grossReturn: rt_avg - k169, isTaxDeffered: true});
    opps.push({field: 'plan_529',              title: '529 Plan',
               balance: 14000,                 grossReturn: rt_avg - k170, isTaxExempt: true});
    opps.push({field: 'taxable_investments',   title: 'Taxable investments',
               balance: 9999999,               grossReturn: rt_avg - k171});
    opps.push({field: 'bank_savings',          title: 'Bank Savings', balance: (efundTarget>0 ? efundTarget : 9999999),
               grossReturn: rt_cash - 0.01, priority: (efundTarget > 0)});
    ...
    opps.forEach((opp) => {
      ...
      opp.annualReturn = opp.grossReturn * (opp.isTaxExempt ? 1 - taxRate : 1);

      opp.fv = FV(opp.annualReturn, firstRetirementYear, 0, -opp.pv, 0); // FV of $1
      if (opp.isTaxDeffered){ opp.fv -= (opp.fv-1)*taxRateRetirement }

      opp.netReturn = opp.balance > 0 ? parseFloat((Math.pow(opp.fv, 1/firstRetirementYear)- 1).toFixed(4)) : 0;
    ...
    opps.forEach((opp, i) => {
      ...
      opp.allocMinPmnt = totalDebtBal > 0 ? parseFloat(((opp.debtBal / totalDebtBal)*minPmnts).toFixed(2)) : 0
